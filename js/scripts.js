  ...
  try {
    ...
    if (headerResponse.ok) {
      ...
      if (headerPlaceholder) {
        ...
        if (doc.body) headerPlaceholder.replaceChildren(...doc.body.childNodes);
      }
    }
    ...
    if (footerResponse.ok) {
      ...
      if (footerPlaceholder) {
        ...
        if (doc.body) footerPlaceholder.replaceChildren(...doc.body.childNodes);
      }
    }
  } catch (error) {
...
function initLiveClock() {
  ...
  function update() {
    const now = new Date();
    if (clock) clock.textContent = now.toLocaleTimeString('en-US', { timeZone: 'America/Denver', hour12: false }) + ' MST';
...
function initInteractiveCLI() {
  ...
  if (termPanel) {
    ...
    if (inputRow) termPanel.insertBefore(chips, inputRow);
  ...
  function processCommand(cmd) {
    ...
    if (trimmed === '') {
      ...
      setTimeout(() => {
        if (typeof window !== 'undefined' && window._triggerGodMode) window._triggerGodMode();
        cliBody.scrollTop = cliBody.scrollHeight;
      }, 400);
  ...
  cliInput.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      ...
      else { histIdx = -1; cliInput.value = ''; return; }
      cliInput.value = history[histIdx] || '';
    }
  });
...
function initCommandPalette() {
  ...
  function resetFilter() {
    allResults().forEach(r => { r.style.display = ''; r.classList.remove('active'); });
  ...
  document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Enter') { e.preventDefault(); activate(getActive()); return; }
    if (e.key === 'ArrowDown') {
      e.preventDefault();
...
async function initLiveStats() {
  ...
  try {
    ...
    if (footerHash && commits[0]) {
      ...
      if (commits[0].html_url) footerHash.href = commits[0].html_url;
    }
    ...
    if (statusCenter) statusCenter.textContent = '● ALL SYSTEMS OPERATIONAL';
    if (statusDot) {
...
export function showToast(msg, duration = 3500) {
  ...
  if (!toast) {
    ...
    if (!toast || !toast.style) return; // guard: test environment mock
    toast.id = '_egg-toast';
    toast.style.cssText = [
  ...
  document.querySelectorAll('.metric').forEach(card => {
    ...
    if (val && val.textContent.includes('∞')) {
      ...
      card.addEventListener('click', () => {
        ...
        coffeeTimer = setTimeout(() => { coffeeClicks = 0; }, 1500);
        ...
        setTimeout(() => { val.style.color = ''; val.style.transform = ''; }, 700);
        if (coffeeClicks >= 3) {
          coffeeClicks = 0;
    ...
    heroTitle.addEventListener('click', () => {
      ...
      if (titleClicks >= 3) {
        ...
        const iv = setInterval(() => {
          ...
          if (iter >= plain.length) { clearInterval(iv); heroTitle.innerHTML = original; }
          iter += 0.7;
        }, 28);
  ...
  if (typeof window !== 'undefined') {
    ...
    if (params.get('hired') === 'true') {
      setTimeout(() => {
        ...
        if (document.getElementById('_godStyle')) return;
        const s = document.createElement('style');
        s.id = '_godStyle';
        s.textContent = `@keyframes _godFlash {
          0%   { opacity:0; transform:scale(0.92); }
