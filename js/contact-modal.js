...
function init() {
  ...
  if (openSection)  openSection.addEventListener('click', openModal);
  ...
  function clearErrors() {
    ['name', 'email', 'phone', 'message'].forEach(f => {
      ...
      if (el)  el.classList.remove('contact-input-error');
  ...
  function setError(field, msg) {
    ...
    if (el)  el.classList.add('contact-input-error');
  ...
  form.addEventListener('submit', async e => {
    ...
    else if (!isValidName(name))    { setError('name',    'Must be at least 2 characters');     valid = false; }
    ...
    if (phone && !isValidPhone(phone)) { setError('phone', 'Invalid phone format');              valid = false; }
    if (!message)                   { setError('message', 'Message is required');                valid = false; }
    else if (!isValidMessage(message)) { setError('message', 'Between 5–5000 characters');      valid = false; }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (sub) { sub.textContent = data.message || 'Failed to send. Try again.'; sub.hidden = false; }
      }
    } catch {
      const sub = document.getElementById('cm-submit-error');
      if (sub) { sub.textContent = 'Network error — please try again.'; sub.hidden = false; }
    } finally {
      btn.disabled        = false;
